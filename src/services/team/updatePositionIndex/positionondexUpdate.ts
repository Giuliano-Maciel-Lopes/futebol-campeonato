...
import { TeamFormation } from "@/schemazod/team/updatePositionindex";
...
export async function UpdatePositionIndexTeam({ userId, id, data }: Props) {
  ...
  if (!player.captainOf || player.captainOf.id !== team.id) {
    throw new AppError("Apenas o capitão deste time pode fazer alteraçoes na escalaçao", 403);
  }
  ...
  await prisma.$transaction(
    players.map((p) => {
      ...
      return prisma.player.update({
        where: { id: p.id },
        data: { positionIndex: newPos! + 1000 },
      });
    })
  );

  // Etapa 2: mover para posições finais corretas
  await prisma.$transaction(
    players.map((p) => {
      const newPos = data.players.find((d) => d.id === p.id)?.positionIndex;
      return prisma.player.update({
